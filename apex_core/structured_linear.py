...
class StructuredLinear(nn.Module):
    ...
    def convert_to_dense_weight(self):
        factory_kwargs = {'device': self.blkdiag1.device, 'dtype': torch.float32}
        dense_weight = self.forward_matmul(torch.eye(self.in_features, **factory_kwargs).to(self.blkdiag1.dtype)).T
        return dense_weight.to(torch.bfloat16)
