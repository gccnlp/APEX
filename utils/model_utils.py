...
def create_hf_model(model_class,
    ...
    #model_config.use_vanilla_attention = use_vanilla_attention
...
def get_optimizer_grouped_parameters(
    ...
    trainable_name_list=['s2', 'mixer_vec','mixer_scale'],
    mixer_name_list=["blkdiag1","blkdiag2"],
):  
    optimizer_grouped_parameters = [
        {
            "params": [
                ...
                if (not any(nd in n.lower() for nd in no_decay_name_list)
                    and p.requires_grad and not any(nd in n.lower() for nd in trainable_name_list) and not any(nd in n.lower() for nd in mixer_name_list))
            ],
        ...
        },
        {
            "params": [
                p for n, p in model.named_parameters()
                if (not any(nd in n.lower() for nd in no_decay_name_list)
                    and p.requires_grad and any(nd in n.lower() for nd in mixer_name_list))
            ],
            "weight_decay": weight_decay,
            "lr": learning_rate*mixer_lr_scale,
        },
        {
            "params": [
                p for n, p in model.named_parameters()
